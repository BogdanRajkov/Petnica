...
def izbacivanje(days, masa_broda):
    date = dt.datetime(2019, 1, 1, 0, 0, 0) + dt.timedelta(days=int(days))
...
def polozaj_planeta(index, t, r_speed=False, matrica=False):  # kao sadasnjost se racuna godina 2000.
    ...
    if matrica and not r_speed:
        ...
        return np.array((x1 + (offset - index_vreme * podaci.step_matrica) / podaci.step_matrica * (x2 - x1),
                         y1 + (offset - index_vreme * podaci.step_matrica) / podaci.step_matrica * (y2 - y1)))
    else:
...
def a_gravitacija(t, r_):  # vraca niz ubrzanja od sva
    ...
    for i in range(n):
        ...
        rez_tren = polozaj_planeta(indeksi[i], t, matrica=False)
    ...
    return np.array((np.sum(a_, axis=0), e_previ_new, (-_rel_pol[min_index], podaci.indeksi[min_index]), dist_dest))


def tip_putanje(index, rel_r_, rel_v_):
    energy = modulo(rel_v_)**2/2 - podaci.grav_par[index]/modulo(rel_r_)
    a = -podaci.grav_par[index]/energy
    if energy < 0:
        momentum = np.cross(rel_r_, rel_v_)
        b = momentum * math.sqrt(a/podaci.grav_par[index])
        e = math.sqrt(1 - (b/a)**2)
        rmin = (1-e) * a
        rmax = (1+e) * a
        return 'elipsa', (rmin, rmax)
    else:
        return 'hiperbola', ()
...
def simulacija(days, fuel_mass, uglovi, snaga, y_max):
    fuel_mass = podaci.min_fuel_mass + fuel_mass / 256 * (podaci.max_fuel_mass - podaci.min_fuel_mass)
    brod = np.array([podaci.dry_mass, fuel_mass])
    r_, v_ = izbacivanje(days, np.sum(brod))
    time_offset = (dt.datetime(2019, 1, 1, 0, 0, 0) + dt.timedelta(days=int(days)) - podaci.beg_of_time).total_seconds()
    ...
    while time < t_max:
        ...
        if fuel_mass <= 0:
            motor_uklj = False
        a_, e_previ, (min_dist_, min_index), curr_dist_dest = a_gravitacija(time + time_offset, r_)
        ...
        if modulo(min_dist_) <= podaci.planet_soi[min_index] and podaci.soi_index == -1:
            print('priblizavam se' + str(min_index) + '. planeti')
            podaci.soi_index = min_index
            v_planet = polozaj_planeta(min_index, time + time_offset, r_speed=True)
            v_planet = v_planet[1], v_planet[3]
            putanja, data = tip_putanje(min_index, min_dist_, v_ - v_planet)
            if putanja == 'elipsa':
                return _r[:i - 1], _v[:i - 1], _step[:i - 1], min_dist_dest
        elif modulo(min_dist_) > podaci.planet_soi[min_index]:
            print('izlazak iz planetine orbite')
            podaci.soi_index = -1
