    ...
    if rank == 0:
        ...
        for i in range(broj_gen):
            ...
            while waiting > 0:
                ...
                waiting = waiting-1
            pop_fit1 = pop_fit
            pop_bit = float_to_bit(days, fuel_mass, koeficijenti, snaga, pop_fit)
            if i % 10 == 9:
            ...
            print('fitness: ', np.all((pop_fit1 - pop_fit) == np.zeros(broj_jedinki)))
            for it in range(broj_jedinki):
                days[it] = days[it] % max_time_span
            f = open('fitness.txt', 'a')
            f.write(str(np.amin(pop_fit))+' '+str(np.median(pop_fit))+'\n')
            f.close()
        for i in range(1, n):
