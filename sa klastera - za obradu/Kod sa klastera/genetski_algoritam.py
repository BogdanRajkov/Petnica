...
def fitnes(min_dist):
    ...
    return np.amax(_t) + np.sum(np.multiply(podaci.r_maks, maks)) + np.sum(np.multiply(podaci.r_min, mini))
...
def elitizam(populacija, pop_elita, p_elit):  # populacija je lista koja cuva podatke za sve jedinke u populaciji.
    ...
    populacija = np.array(sorted(populacija, key=lambda x: x[-1], reverse=False))
...
# def kros_over(kod1, kod2, br_jed):
#    n = np.random.random_integers(br_jed)
#    return np.concatenate((kod1[:n], kod2[n:])), np.concatenate((kod2[:n], kod1[n:]))
...
def genetski_algoritam(populacija, pop_elita, p_elit, p_mut):
    ...
    populacija = populacija[:podaci.broj_jedinki//4]
    #populacija = selekcija(populacija, fitness)
    populacija1 = ukrstanje(populacija)
    populacija2 = ukrstanje(populacija)
    populacija3 = ukrstanje(populacija)
    populacija4 = ukrstanje(populacija)
    #populacija5 = ukrstanje(populacija)
    #populacija6 = ukrstanje(populacija)
    #populacija7 = ukrstanje(populacija)
    #populacija8 = ukrstanje(populacija)
    populacija = np.concatenate((populacija1, populacija2, populacija3, populacija4))
